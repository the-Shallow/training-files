...
void main(){
	...
	while(1){
		...
		switch(choice){
			...
			case 4:{
				int item;
				printf("\nEnter the value of the element you want to insert at bottom\n");
				scanf("%d",&item);
				if(insert_bottom(stack,item,size) == 0) printf("\nStack is Full\n");
				else printf("\n%d is inserted at the bottom of the stack\n",item);
				break;
			}
		}
