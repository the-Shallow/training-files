...
int remove_from_middle(int stack[] ,int size){
 ...
 if( top == size/2 ){
  ...
  int curr = pop(stack);
  int element = remove_from_middle(stack,size);
...
void main(){
	...
	while(1){
		...
		switch(choice){
			...
			case 4:{
				int item = remove_from_middle(stack,size); 
				if( item == -1) printf("\nStack is Empty\n");
				else printf("\n%d is removed at the bottom of the stack\n",item);
				break;
			}
		}
